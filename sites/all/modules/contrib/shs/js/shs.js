    ...
    $.ajax({
      ...
      success: function(data) {
        if (data.success == true) {
          ...
          if (((parent_value[0] != 0) && ((data.data.length == 0) || ((data.data.length == 1 && !data.data[0].tid)))) && !(settings.settings.create_new_terms && (settings.settings.create_new_levels || (parent_value[0] == settings.any_value && default_value == 0)))) {
            // Remove element.
            $element.remove();
            return;
          }
